...
A = np.array([[1,1,1,0],
              ...
              [1,0,0,2/3],
              [4,0.5,0,2/3]])
Asol = np.matmul(np.linalg.inv(A),np.array([[ca/1000],[cb/1000],[cc/1000],[cd/1000]]))
print(Asol)
...
ss.add_element(location=[[L, 0.0], [L+L/2,0.0]], EI = EI)
ss.add_element(location=[[L+L/2, 0.0], [2*L,0.0]], EI = EI)

# Añadimos cargas
ss.q_load(element_id=1,  q=(qa+qb,qa+qb/2))
ss.q_load(element_id=2,  q=(qb/2,qb/4))
ss.q_load(element_id=3,  q=(qb/4,0))
